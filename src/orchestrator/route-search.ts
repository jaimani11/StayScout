 ...
 *   - whether MockItalyProvider has a curated match for the destination
 *
...
export function routeForIntent(intent: TripIntent, registry: RouteRegistry): RouteDecision {
  ...
  if (MockItalyProvider.knowsDestination(destination)) {
    return { kind: 'inventory', providers: [MockItalyProvider] };
  }
